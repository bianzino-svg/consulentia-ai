...
from services.auth import authenticate_user, create_user, get_user_by_id, init_db, list_reports_for_user
from services.dashboard_service import get_dashboard_bundle, generate_user_report

BASE_DIR = Path(__file__).resolve().parent
templates = Jinja2Templates(directory=str(BASE_DIR / 'templates'))

app = FastAPI(title='ConsulentIA AI B2C MVP')
app.add_middleware(SessionMiddleware, secret_key='consulentia-ai-demo-secret-change-me')
...
def register_page(request: Request):
    ...
    return templates.TemplateResponse(request, 'register.html', {'request': request, 'error': None, 'user': None})


@app.post('/register', response_class=HTMLResponse)
def register(request: Request, full_name: str = Form(...), email: str = Form(...), password: str = Form(...)):
    if len(password) < 6:
        return templates.TemplateResponse(request, 'register.html', {'request': request, 'error': 'La password deve avere almeno 6 caratteri.', 'user': None})
    user_id = create_user(full_name, email, password)
    if user_id is None:
        return templates.TemplateResponse(request, 'register.html', {'request': request, 'error': 'Esiste già un account con questa email.', 'user': None})
...
def login(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if not user:
        return templates.TemplateResponse(request, 'login.html', {'request': request, 'error': 'Credenziali non valide.', 'user': None})
...
def dashboard(request: Request, profile: str = 'bilanciato'):
    ...
    return templates.TemplateResponse(request, 'dashboard.html', {'request': request, 'user': user, 'profile': profile, 'bundle': bundle, 'page': 'dashboard'})
...
def reports(request: Request):
    ...
    return templates.TemplateResponse(request, 'reports.html', {'request': request, 'user': user, 'reports': report_rows, 'page': 'reports'})
