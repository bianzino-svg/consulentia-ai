        ...
        with get_connection() as conn:
            if using_postgres():
                cur = _execute(
                    conn,
                    """
                    INSERT INTO users (full_name, email, password_hash, access_code, access_status)
                    VALUES (?, ?, ?, ?, ?)
                    RETURNING id
                    """,
                ...
                cur = conn.execute(
                    "INSERT INTO users (full_name, email, password_hash, access_code, access_status) VALUES (?, ?, ?, ?, ?)",
                    (full_name.strip(), email.strip().lower(), password_hash, code, access_status),
                )
                conn.commit()
...
def request_access(full_name: str, email: str):
    ...
    user_id = create_user(
        full_name=full_name or "Cliente",
        email=clean_email,
...
def get_user_by_email(email: str):
    with get_connection() as conn:
        cur = _execute(conn, "SELECT * FROM users WHERE email = ?", (email.strip().lower(),))
        return cur.fetchone()
...
def authenticate_user(email: str, password: str):
    ...
    if not user:
        return None
    if not verify_password(password, _row_get(user, "password_hash", "")):
        return None
    return user
...
def authenticate_access_code(email: str, access_code: str):
    ...
    saved = str(_row_get(user, "access_code", "") or "").strip().upper()
    submitted = str(access_code or "").strip().upper()

    if not saved or saved != submitted:
        return None
...
def count_reports_for_user(user_id: int) -> int:
    with get_connection() as conn:
        cur = _execute(conn, "SELECT COUNT(*) AS total FROM reports WHERE user_id = ?", (user_id,))
        row = cur.fetchone()
        return int(_row_get(row, "total", 0)) if row else 0


def set_user_premium(user_id: int, value: int):
    with get_connection() as conn:
        _execute(conn, "UPDATE users SET is_premium = ? WHERE id = ?", (1 if int(value) == 1 else 0, user_id))
        conn.commit()


def set_user_active(user_id: int, value: int):
    with get_connection() as conn:
        _execute(conn, "UPDATE users SET access_status = ? WHERE id = ?", ("active" if int(value) == 1 else "pending", user_id))
        conn.commit()
...
def regenerate_access_code(user_id: int) -> str:
    ...
    with get_connection() as conn:
        _execute(conn, "UPDATE users SET access_code = ? WHERE id = ?", (new_code, user_id))
        conn.commit()
    return new_code
...
def is_user_premium(user_id: int) -> bool:
    with get_connection() as conn:
        cur = _execute(conn, "SELECT COALESCE(is_premium, 0) AS is_premium FROM users WHERE id = ?", (user_id,))
        row = cur.fetchone()
        return bool(row and int(_row_get(row, "is_premium", 0)) == 1)
    ...
    return [
        {
            ...
            "report_count": int(_row_get(row, "report_count", 0)),
            "is_premium": int(_row_get(row, "is_premium", 0)) == 1,
            "plan": "Premium" if int(_row_get(row, "is_premium", 0)) == 1 else "Free",
            "access_code": _row_get(row, "access_code", ""),
            "access_status": _row_get(row, "access_status", "pending"),
        }
        for row in rows
    ]
