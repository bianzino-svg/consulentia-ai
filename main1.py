...
def notify_admin_limit_reached(user):
    try:
        send_otp_email(
            ADMIN_EMAIL,
            f"Utente arrivato al limite Free: {user['email']}",
        )
    except Exception:
        pass
...
def send_code(request: Request, email: str = Form(...)):
    ...
    otp_storage[clean_email] = code
    send_otp_email(clean_email, code)
    return templates.TemplateResponse(
        request,
        "otp_verify.html",
        {"request": request, "email": clean_email, "user": current_user(request)},
    )
...
def verify_code(request: Request, email: str = Form(...), code: str = Form(...)):
    clean_email = email.strip().lower()
    saved_code = otp_storage.get(clean_email)
    ...
    if not user:
        user_id = create_user("Cliente", clean_email, "otp-login-user")
    else:
        user_id = int(user["id"])
...
def register(
    ...
    if len(password) < 6:
        return templates.TemplateResponse(
            request,
            "register.html",
            {"request": request, "error": "La password deve avere almeno 6 caratteri.", "user": None},
        )
    ...
    if user_id is None:
        return templates.TemplateResponse(
            request,
            "register.html",
            {"request": request, "error": "Esiste già un account con questa email.", "user": None},
        )
    request.session.clear()
    request.session["user_id"] = int(user_id)
    return RedirectResponse("/app/dashboard", status_code=302)
...
def login_page(request: Request):
    return templates.TemplateResponse(
        request,
        "login.html",
        {"request": request, "error": None, "user": current_user(request)},
    )
...
def dashboard(request: Request, profile: str = "bilanciato"):
    ...
    return templates.TemplateResponse(
        ...
        "dashboard.html",
        {
            ...
            "bundle": bundle,
            "page": "dashboard",
            "is_limited": is_limited_free_user(user),
        },
    )
...
def fractals(request: Request, profile: str = "bilanciato"):
    ...
    if is_limited_free_user(user):
        notify_admin_limit_reached(user)
        return render_limit_page(request, user)
    bundle = get_dashboard_bundle(profile)
    return templates.TemplateResponse(
        request,
        "fractals.html",
        {"request": request, "user": user, "profile": profile, "bundle": bundle, "page": "fractals"},
    )
...
def macro(request: Request, profile: str = "bilanciato"):
    ...
    return templates.TemplateResponse(
        request,
        "macro.html",
        {"request": request, "user": user, "profile": profile, "bundle": bundle, "page": "macro"},
    )
...
def allocation(request: Request, profile: str = "bilanciato"):
    ...
    return templates.TemplateResponse(
        request,
        "allocation.html",
        {"request": request, "user": user, "profile": profile, "bundle": bundle, "page": "allocation"},
    )
...
def reports(request: Request):
    ...
    return templates.TemplateResponse(
        request,
        "reports.html",
        {"request": request, "user": user, "reports": report_rows, "page": "reports", "is_limited": is_limited_free_user(user)},
    )
...
def admin_users_page(request: Request):
    ...
    return templates.TemplateResponse(
        request,
        "admin_users.html",
        {"request": request, "user": user, "users": users, "page": "admin"},
    )
